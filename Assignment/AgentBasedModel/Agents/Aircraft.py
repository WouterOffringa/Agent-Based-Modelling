...
class Aircraft(object):
    ...
    def __init__(self, flight_id, a_d, start_node, goal_node, spawn_time, nodes_dict):
        ...
        Args:
            ...
            a_d (str): Whether the aircraft is arriving ("a") or departing ("d")
            start_node (int): node_id of start node
            goal_node (int): node_id of goal node
            spawn_time (float): Spawn time of the aircraft
        ...
        self.spawntime = spawn_time     # Spawn time
        self.start = start_node         # Start node_id
    ...
    def get_heading(self, xy_start, xy_next):
        """Determines heading of an aircraft based on a start and end xy position. Heading is stored in self.heading.
    ...
    def request_taxibot(self, nodes_dict, taxibot_list, heuristics, t):
        ...
        Args:
            nodes_dict (dict): Dictionary of the nodes
            taxibot_list (list): List containing all taxibots
            heuristics (dict): Dictionary of the heuristics
            t (float): Current time
        """        
        # Initialize the list of the traveltimes from each taxibot to the arrived
        available_taxibots = [] # List of available taxibots
        traveltime_list = []    # List of taxibot travel time to the aircraft

        for taxibot in taxibot_list:
            if taxibot.status == "holding" or taxibot.status == "taxiing, available":
                # Determine the travel time
                success, route_to_ac = simple_single_agent_astar(nodes_dict, taxibot.from_to[0], self.start, heuristics, t)

                # Log the results
                traveltime_list.append(route_to_ac[-1][1])
                available_taxibots.append(taxibot)

        if traveltime_list != []:
            # Determine which taxibot has the shortest travel time and assign it as winning
            winning_tug = available_taxibots[traveltime_list.index(min(traveltime_list))]
    ...
    def track_delay_waiting(self, t):
        """Keep track of the delay from waiting on a taxibot.
    ...
    def remove_from(self, *lists):
        """Delete self from the desired lists, if present.
        """        
        for lst in lists:
            if self in lst:
                lst.remove(self)